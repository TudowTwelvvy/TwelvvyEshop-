...
function Nav() {
  ...
  return (
    <div className=" flex flex-1 text-white px-2  items-center ">
      <Link to="/">
        <div className=" relative w-[230px] max-lg:w-[90px] border border-transparent hover:border-gray-500 m-2">
          <div className="flex  justify-start  -mr-[2px]  m-2 ">
            <img
              src={logo}
              alt="logo"
              className="w-[100px] h-[70px] object-contain max-lg:w-[60px] max-lg:h-[60px] "
            />

            <h1 className="max-lg:hidden text-xl font-semibold text-red absolute  mt-[32px] ml-2 right-0 bottom-[30px] ">
              Twelvvy<span className="text-red-500 text-3xl">E</span>Shop.
            </h1>
          </div>
        </div>
      </Link>

      <div className=" hidden lg:flex h-10 rounded-lg  flex-grow items-center bg-white relative py-2 ml-2 w-[20%]">
        <span
          className="text-black border-r border-gray-500 px-2 cursor-pointer  font-semibold"
            ...
            <ul className="bg-white absolute left-0 -bottom-6 h-80 w-56 top-10 overflow-y-auto overflow-x-hidden border-[1px] border-gray-500 text-black p-2 flex flex-col gap-1 z-50">
              {allItems.map((item) => {
                return (
                  <li
                    key={item._id}
                    className="text-sm tracking-wide font-titleFont border-b-[1px] border-b-transparent hover:border-b-gray-500 cursor-pointer duration-200"
                  >
                    {item.title}
                  </li>
                )
        ...
        <div className="w-full flex items-center">
          <input
            type="text"
            className="w-full mx-2 outline-none text-base text-black"
          />
      ...
      <div className="flex flex-1 items-center justify-evenly ml-2 -">
        <Link to="/signin">
          <div className="flex flex-col p-2  headerHover">
            {userInfo ? (
              <p className=" text-md text-gray-200 font-bold capitalize tracking-wide">
                {userInfo.userName}
              </p>
            ) : (
              <p className=" max-lg:text-sm pl-0">
                <span className="hidden md:block">Hello,</span> sign in
              </p>
            )}
            <p className="flex font-semibold items-center   max-lg:hidden">
              Accounts & lists{' '}
              <span>
        ...
        <Link to="/cart">
          <div className="relative p-2 headerHover">
        ...
        {userInfo && (
          <div
            className="flex flex-col justify-center items-center headerHover relative text-red-500"
